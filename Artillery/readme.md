...
3. Формируем запрос для построения отчета.
...
#### Домашнее задание
1. Создать таблицу для учета [`Скорости среднего ветра`](./_Docs/AlgoritmBp.md)(**Таблица 3**) для расчета с помощью ветрового ружья
2. Написать новый отчет и оформить его в виде `View` и использованием `CTE`. 

**Самая эффективная высота измерения**
